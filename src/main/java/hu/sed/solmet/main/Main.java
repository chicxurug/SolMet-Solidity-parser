...
public class Main {
  ...
  private static Options createCmdLineOptions() {
    ...
    options.addOption(
        Option.builder("outFile").hasArg().argName("name").desc("output file").required().build());
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      ...
      try (BufferedWriter writer =
              Files.newBufferedWriter(Paths.get(cmdLine.getOptionValue("outFile")));
        ...
        for (String solPath : solPaths) {
          ...
          for (ContractDefinitionContext contract : metrics.keySet()) {
            ...
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[4] / metrics.get(contract)[3]);
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[5] / metrics.get(contract)[3]);
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[6] / metrics.get(contract)[3]);
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[7] / metrics.get(contract)[3]);
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[8] / metrics.get(contract)[3]);
            record.add(
                metrics.get(contract)[3] == 0
                    ? 0f
                    : (double) metrics.get(contract)[14] / metrics.get(contract)[3]);
