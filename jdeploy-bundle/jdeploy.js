...
var jarName = "SolMet-1.0-SNAPSHOT.jar";
...
if (env["JAVA_HOME"]) {
  env["PATH"] =
    env["JAVA_HOME"] + path.sep + "bin" + path.delimiter + env["PATH"];
}
...
if (javaVersion === false || javaVersion < 1.8 || env["JDEPLOY_USE_NODE_JRE"]) {
  ...
  if (!test("-e", packageJson)) {
    fs.writeFileSync(
      packageJson,
      JSON.stringify({ name: "jdeploy-support", version: "1.0.0" }),
      "utf8"
    );
  }
